...
export default function RecentRecords({ providerId }: { providerId?: number }) {
          ...
          <tbody>
            ...
            {records.map((r) => (
              <tr key={r.id} className="border-b border-gray-700/50 last:border-0">
                <td className="py-2 text-gray-400">{new Date(r.timestamp).toLocaleString()}</td>
                ...
                <td className="py-2 text-right text-emerald-400 font-medium">${r.cost_usd.toFixed(6)}</td>
              </tr>
            ))}
          </tbody>
        </table>
